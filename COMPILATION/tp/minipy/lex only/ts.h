		...
		t2 = t->suiv;
			    while(t1!=NULL){
				   if(strcmp(entite,t1->name)==0) { 
					   ...
					   if(t2==NULL) {t1->suiv = inserer(entite,code,type,val,NULL,t2,0).pidf; boo = 0;}
						else {t2=t2->suiv;}
						t1=t1->suiv;
				   }
			    }
				   if(t->suiv == NULL){
					   if(boo == 1)
					   t->suiv = inserer(entite,code,type,val,NULL,t1,0).pidf;
				   }else{
						if( boo == 1 ) t1= inserer(entite,code,type,val,NULL,t2,0).pidf;
					   }       
		}else{
		...
		p2 = p->suiv;					
				while(p1!=NULL){	
				   if(strcmp(entite,p1->name)==0) { 
					   ...
					   if(p2==NULL) {p1->suiv = inserer(entite,code,type,val,p2,NULL,1).pmc; boo = 0;}
					     else {p2=p2->suiv;}
						p1=p1->suiv;
				   }
				}
				   if(p->suiv == NULL){
					   if( boo == 1 )
					   p1->suiv = inserer(entite,code,type,val,p1,NULL,1).pmc;
				   }else{
					   if( boo == 1 ) p1= inserer(entite,code,type,val,p2,NULL,1).pmc;
					   }       
		}else{
		...
		p2 = p->suiv;
			   while(p1!=NULL){
				   if(strcmp(entite,p1->name)==0){ 
					   ...
					   if(p2==NULL) {p1->suiv = inserer(entite,code,type,val,p2,NULL,2).psep; boo = 0;}
							else {p2=p2->suiv;}
					    p1=p1->suiv;
				   }
			    }
				   if(p->suiv == NULL){
					   if(boo == 1)
					   p->suiv = inserer(entite,code,type,val,p1,NULL,2).psep;
				   }else{
						   if( boo == 1 ) p1= inserer(entite,code,type,val,p2,NULL,2).psep;
					   }       
		}else{
